...
- **Video-Specific Fields**:
  - "videoHighlights": An array of 3-5 key moments. If the transcript HAS timestamps (e.g. [2:30]), include the timestamp and a one-sentence description. If no timestamps are available, use your internal knowledge of the specific video (if known) or common themes from the creator/topic to provide the 3 most important THEMES.
  - "speakers": Identify the host/creator (e.g., from the channel name) and any guests mentioned.
- **Speculation & Synthesis**: If the transcript is sparse or generic (autogenerated), you MUST synthesize a deeper meaning. For example, if it's a Mark Manson video, apply your knowledge of his 'Counter-Intuitive' philosophy to interpret the metadata. Do NOT output a generic or shallow disclaimer.
- **Structure**:
