...
from reminder_service import calculate_next_reminder, run_reminder_check
...
from rate_limit import check_rate_limit, client_ip, _RATE_LIMITS, _PAID_BUCKETS
...
# The per-bucket limits (_RATE_LIMITS) and paid-bucket set (_PAID_BUCKETS) live
# in rate_limit.py so webhook.py can share them without importing this module.
...
# Shared analysis transforms (read-time, scraped-content analysis, YouTube
# metadata, image storage) live in analysis_shared.py so background.py can use
# them without importing this entrypoint.
...
# WhatsApp Webhook
# ─────────────────────────────────────────────
# The webhook lives in webhook.py. Re-exported here because Firebase discovers
# deployed functions by scanning this entrypoint module's namespace.
from webhook import whatsapp_webhook  # noqa: F401


# ─────────────────────────────────────────────
# Background Processing
# ─────────────────────────────────────────────
# The processor lives in background.py. Re-exported here because Firebase
# discovers deployed functions by scanning this entrypoint module's namespace.
from background import process_link_background  # noqa: F401
