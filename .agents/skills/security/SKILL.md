...
> **Codex notes (ported from `.claude/skills/security`).** Invoke as
> `$security <area>`. `/security-review` is a Claude built-in that does not
> exist here; skip that seeding step and do the eight-lens sweep directly.
> `/onboard` and `/ship` refer to the sibling skills in `.agents/skills/`.
