...
> **Codex notes (ported from `.claude/skills/polish`).** Invoke as `$polish <screen>`.
> Where the text says `/onboard` or `/ship`, use the sibling skills in
> `.agents/skills/`. `send_later` does not exist: after triggering TestFlight,
> `gh run watch` the run and report the build number in the same turn. The
> Playwright render harness works the same (`/opt/pw-browsers` exists only in
> the Claude cloud container; locally use `npx playwright install chromium`).
...
On the current `codex/*` branch. House rules: theme tokens only (`text-text`,
