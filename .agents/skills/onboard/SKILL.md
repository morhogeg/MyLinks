...
> **Codex notes (ported from `.claude/skills/onboard`).** Invoke by mentioning
> `$onboard` (or via `/skills`) with the request as the argument. Where the text
> says "current `claude/*` branch" read "current `codex/*` branch". `/ship`,
> `/polish`, `/security` refer to the sibling skills in `.agents/skills/`.
