...
- **2026-07-20 (latest) — MY NOTES: a central view of every personal note with
  ...
  `.env.local` in cloud sessions). Not yet shipped — merge via `/ship`. Stats
  note counts ride the existing per-session Insights cache, so a note added
  mid-session shows on the next session's Insights (same as every other stat).
