    ...
    return (
        <div className="relative inline-block" ref={containerRef}>
            <input
                ref={inputRef}
                type="text"
                className={`uppercase font-black tracking-widest px-2 py-0.5 rounded-lg outline-none focus:ring-1 focus:ring-accent/50 bg-white-fixed/10 ${className}`}
                style={{
            ...
            {isOpen && (value.trim() !== '' || filteredCategories.length > 0) && (
                <div
                    className="absolute top-full start-0 mt-1 w-48 bg-card border border-white-fixed/10 rounded-lg shadow-xl z-50 overflow-hidden flex flex-col py-1 animate-in fade-in zoom-in-95 duration-200 max-h-60 overflow-y-auto"
                    onClick={(e) => e.stopPropagation()}
                >
                    {filteredCategories.map(category => (
                        <button
                            ...
                            className="flex items-center justify-between px-3 py-2 text-xs hover:bg-white-fixed/5 text-start w-full transition-colors group"
                        >
                            <span
                    ...
                    {isNew && (
                        <button
                            ...
                            className="flex items-center gap-2 px-3 py-2 text-xs hover:bg-accent/10 text-start w-full transition-colors border-t border-white-fixed/5 text-accent font-bold"
                        >
