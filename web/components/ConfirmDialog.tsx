...
export default function ConfirmDialog({
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 animate-fade-in">
            {/* Backdrop */}
            <div
                className="absolute inset-0 bg-black-fixed/60 backdrop-blur-sm"
                onClick={onClose}
            />
            ...
            <div
                role="alertdialog"
                aria-modal="true"
                className="relative bg-card w-full max-w-md rounded-2xl border border-white-fixed/5 shadow-2xl p-6 overflow-hidden animate-scale-up"
            >
                {/* Header */}
                <div className="flex items-start gap-4 mb-4">
                    ...
                    <div className="flex-1">
                        <h3 className="text-xl font-bold text-white-fixed leading-tight">
                            {title}
                        </h3>
                    ...
                    <button
                        onClick={onClose}
                        className="p-1 hover:bg-white-fixed/5 rounded-full text-text-muted transition-colors"
                    >
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <div className="flex gap-3 mt-6">
                    ...
                    <button
                        ...
                        className={`flex-1 px-4 py-2.5 rounded-xl font-medium transition-colors ${variant === 'danger'
                                ? 'bg-red-500 text-white-fixed hover:bg-red-600'
                                : 'bg-white-fixed text-black-fixed hover:bg-gray-200'
                            }`}
                    >
                        {confirmLabel}
                    </button>
